...
@lexer::members {
  ...
  public Token nextToken() {
    ...
    if (_input.LA(1) == EOF && !this.indents.isEmpty()) {
      ...
      this.emit(commonToken(UVLConstraintParser.NEWLINE, "\n"));
      ...
      this.emit(commonToken(UVLConstraintParser.EOF, "<EOF>"));
    }
  ...
  private Token createDedent() {
    CommonToken dedent = commonToken(UVLConstraintParser.DEDENT, "");
   ...
   )
   {
     ...
     if (opened > 0 || next == '\r' || next == '\n' || next == '/' && nextNext == '/') {
       ...
       if (indent == previous) {
         ...
         indents.push(indent);
         emit(commonToken(UVLConstraintParser.INDENT, spaces));
       }
       else {
